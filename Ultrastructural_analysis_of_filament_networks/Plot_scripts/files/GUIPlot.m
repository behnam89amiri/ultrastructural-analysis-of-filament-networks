...
[file,path]=uigetfile('*.mat','Select a filopodium file from analysis results');
cd(path);
load(file);
cd(currentfolder)
Cols=parula(50);
...
handles.Length=filopmetrics.Length;
handles.Thickness=filopmetrics.Thickness;
handles.Area=filopmetrics.FilamentAreaAVG;
handles.Number=filopmetrics.FilamentNumber;
...
text(0.01,0.8,['Length: ' num2str(round(handles.Length)) 'nm'],'Units','normalized','FontUnits','normalized')
text(0.01,0.6,['Thickness: ' num2str(round(handles.Thickness))  'nm'],'Units','normalized','FontUnits','normalized')
text(0.01,0.4,['Average cross-sectional area: ' num2str(round(handles.Area))  'nm^2'],'Units','normalized','FontUnits','normalized')
...
switch str{val};
    case 'length'
        Prop=1;
    case 'angle to the axis of filopodium'
        Prop=2;
    case 'bendiness'
        Prop=3;
...
switch Prop;
    case 1
        ...
        for j=1:length(Data)
            col=[interp1(refProp,Cols(:,1)',max(minProp,min(maxProp,Lengths(j))) )    interp1(refProp,Cols(:,2)',max(minProp,min(maxProp,Lengths(j))))    interp1(refProp,Cols(:,3)',max(minProp,min(maxProp,Lengths(j))))];
            %     dataFil=[Data(j).X';Data(j).Y';Data(j).Z'];
            plot3(Data(j).X,Data(j).Y,Data(j).Z,'color',col,'LineWidth',filamentThickness)
        end
        box on
        ctitle='length (nm)';
